...
def main():
    ...
    """)
    writer_con.raw_sql(
        "INSERT INTO concurrent_test VALUES (1, 'initial'), (2, 'data');"
    )

    def reader_udxf(df: pd.DataFrame):
        reader_con = xo.duckdb.connect(db_path)
        return (
            reader_con.tables["concurrent_test"]
            .agg(row_count=xo._.id.count())
            .execute()
        )
    ...
    client.do_action(
        AddExchangeAction.name, duckdb_reader_udxf, options=client._options
    )

    print("multi-DuckDB Flight server started at grpc://localhost:8815")
