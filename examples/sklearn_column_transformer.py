...
# Filter numerical warnings from sklearn on demo data
...
def create_mixed_data(include_missing=False):
    ...
    # Create realistic data with actual signal
    age = np.clip(np.random.normal(42, 12, n_samples), 18, 80)
    income = np.clip(np.random.normal(55000, 20000, n_samples), 25000, 120000)
    credit_score = np.clip(np.random.normal(680, 80, n_samples), 300, 850)
    account_balance = np.clip(np.random.normal(8000, 4000, n_samples), 0, 25000)

    # Create target with signal
    approval_score = (
        (income - 55000) / 20000 * 0.35
        + (credit_score - 680) / 80 * 0.40
        + (account_balance - 8000) / 4000 * 0.15
        + (age - 42) / 12 * 0.10
...
def example_column_transformer_step():
    ...
    step = Step.from_instance(preprocessor, name="preprocessor")
...
def example_full_pipeline_column_transformer():
    ...
    step = Step.from_instance(sklearn_pipeline, name="full_pipeline")
...
def example_passthrough_columns():
    ...
    step = Step.from_instance(preprocessor, name="preprocessor_passthrough")
    all_features = tuple(
...
def example_nested_pipelines():
    ...
    step = Step.from_instance(preprocessor, name="nested_preprocessor")
    all_features = tuple(numeric_features + categorical_features)
    fitted = step.fit(expr, features=all_features)
