...
:::{.callout-tip}
Every `.execute()` re-reads the source and moves the data across again. If you run the pipeline more than once, add `.cache()` right after `.into_backend(...)` so the move only happens on the first run and later runs read the cached copy. See [Cache results by backend](cache_by_backend.qmd).
:::
