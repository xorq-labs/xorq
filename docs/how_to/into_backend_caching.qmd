...
This guide demonstrates how to effectively use Xorq's pipeline operations to switch between different backends, cache results for optimal performance, and leverage ParquetStorage for persistent caching. This guide uses the penguins dataset and works with the default backend, DuckDB, and PostgreSQL.
...
Explore these capabilities using practical examples.

## Setup and initial connections

First, set up the environment and create connections to different backends:
...
Start by loading the famous penguins dataset and exploring its structure:
...
Before demonstrating backend switching, clean the data by removing rows with missing values:
...
Move the cleaned penguins data to DuckDB for processing:
...
Now demonstrate moving data to PostgreSQL for additional processing:
...
Xorq provides sophisticated caching mechanisms to optimize performance and avoid recomputation. Explore different storage types.
