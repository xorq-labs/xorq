...
An in-place `UPDATE` invalidates the cache. Redshift implements `UPDATE` as delete-plus-insert, so it moves the insert and delete counters while leaving the row count unchanged, which a row count alone cannot see.
