...
The `source` backend here only writes the file on a miss and reads it back on a hit; the cache itself lives on disk.
...
- [Cache strategies, storage, and backend invalidation](../concepts/core_concepts/caching_internals.qmd): how keys are computed, plus the `GCSCache` and TTL variants not covered here
