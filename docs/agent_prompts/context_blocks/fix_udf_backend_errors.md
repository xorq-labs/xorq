...
# Solution 2: Simple cache (fallback option)
local_table = remote_table.cache()  # Brings data to local backend
result = local_table.mutate(new_col=my_udf.on_expr(local_table))

# Solution 3: Use into_backend to switch backends
import xorq.api as xo
local_table = remote_table.into_backend(xo.connect())
result = local_table.mutate(new_col=my_udf.on_expr(local_table))

# Solution 4: For aggregations, cache with ParquetCache
from xorq.caching import ParquetCache
...
fitted = xorq_pipeline.fit(table, features=features, target=target)
predictions = fitted.predict(table[features])  # ❌ ValueError: Can't handle RandomForestRegressor
```
...
```
KeyError: 0
File .../xorq/expr/udf.py:49, in make_dunder_func.<locals>.fn_from_arrays(*arrays)
    value = make_pyarrow_array(return_type, value)
File .../pandas/core/frame.py:4113, in DataFrame.__getitem__(self, key)
    indexer = self.columns.get_loc(key)
KeyError: 0
```

**Why it occurs:** When xorq converts UDF output to PyArrow arrays for struct types, it expects a `pd.Series` of dicts, not a DataFrame. If you return a DataFrame, it tries to access by integer index (`df[0]`) instead of column names, causing the KeyError.
...
# Then trying to unpack:
result = table.mutate(
    markup_metrics=markup_udf.on_expr(table),
    MARKUP_30PCT=_.markup_metrics['MARKUP_30PCT']  # Works
).cache(ParquetCache.from_kwargs())  # ❌ ERROR: KeyError: 0 during cache execution
```

**Fix:** Return a `pd.Series` of dictionaries instead:
...
result = table.mutate(
    markup_metrics=markup_udf.on_expr(table),
    MARKUP_30PCT=_.markup_metrics['MARKUP_30PCT'],
    REVENUE_GAP=_.markup_metrics['REVENUE_GAP']
).cache(ParquetCache.from_kwargs())  # ✅ Works!
```
