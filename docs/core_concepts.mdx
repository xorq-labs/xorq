...
LetSQL provides a powerful system for extending query engines with custom User-Defined Functions (UDFs). Here are three key types supported:
...
LetSQL's Ephemeral Flight Service provides a high-performance data transfer mechanism between engines using Apache Arrow Flight. Unlike traditional data transfer methods, this service provides:
...
While LetSQL is built on top of Ibis, it extends its capabilities in several key ways. Here's a comprehensive example showing the differences:
