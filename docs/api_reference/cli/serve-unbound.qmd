...
- **--to-unbind-hash**: Hash of the node to replace with an unbound source.
- **--to-unbind-tag**: Tag of the node to replace (alternative to hash).
...
- If neither `--to-unbind-hash` nor `--to-unbind-tag` is provided, the command attempts to infer the target based on graph analysis; providing one is recommended for determinism.
