...
| `-p`, `--params` | none | Override expression parameters. Format: `key=value`. Repeatable, for example, `-p threshold=0.5 -p region=us`. |
...
Assuming you've built the penguins analysis expression with [`xorq build`](build.qmd):
...
The command executes the UDXF Xorq expression and produces a dataset with the health analysis results.
...
If the expression was built with named parameters (for example, via `xo.param`), override
...
type—for example, `threshold=0.5` becomes a float, `region=us` a string.
