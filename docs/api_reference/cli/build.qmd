...
The Xorq command-line tool provides the `build` command to compile Xorq expressions into reusable artifacts that can be executed later. This separation enables:
...
| `--debug` | off | Output SQL files and other debug artifacts alongside the build. |
...
- `<builds-dir>` is where the artifacts are generated (defaults to "builds")

### Example: Penguins data processing

Create a complete example using the penguins dataset. First, create a script that defines a Xorq expression with a UDXF for data processing:
...
Now, build this expression using the command-line tool:
