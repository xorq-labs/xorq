...
| `XORQ_CACHE_VERIFY_WRITES` | Read each cache write back before publishing it, and refuse to publish one that does not decode to the rows streamed into it | No | `True` |
| `XORQ_LOG_LEVEL` | Log level for file logging (`DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`, or `OFF` to turn off) | No | `INFO` |
