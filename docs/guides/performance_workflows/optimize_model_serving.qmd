...
4. In **Terminal 2**, run the code examples on this page by saving each snippet as a `.py` file (for example, `connection_pool.py`, `singleton_client.py`, etc.) and running them with Python while the server in Terminal 1 stays running.

## Reuse client connections

All examples in this section assume the test Flight server from the previous section is running and exposes an exchange named `"default"`.

Reusing Flight client connections reduces connection overhead and improves latency. Create a connection pool or a singleton client for your application.
...
3. Define a caching prediction function that checks the cache first, and only makes a network request if the result isn't cached. Store results in the cache for future requests.
