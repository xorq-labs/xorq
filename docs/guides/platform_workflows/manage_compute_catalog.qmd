...
If you already have a Python file with your Xorq expression, then you can proceed directly to building. Make sure your expression is assigned to a variable (e.g., `model_expr`, `my_expr`, etc.) that you can reference with the `-e` flag.
...
For example, if you created `train_model.py` with `model_expr`:
...
:::{.callout-important}
The last line (`builds/abc123def456`) is the build hash directory. **Copy this hash** - you'll need it in the next step to add the build to the catalog. Replace `abc123def456` with your actual hash.
:::

## Add builds to the catalog

Add builds to the catalog so you can reference them by alias instead of build hash.

### Add a build with an alias

Add a build directory to the catalog:
...
3. Use the full path if the file is elsewhere: `xorq build /full/path/to/file.py -e <expression_name> --builds-dir builds`
...
3. Add the build if it's missing: `xorq catalog add builds/<build_hash> --alias <alias>`
4. Use the entry ID instead of alias if the alias was removed
...
3. Free up disk space if storage is full
