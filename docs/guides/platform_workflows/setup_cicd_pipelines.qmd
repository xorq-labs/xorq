...
- The `xorq build` command outputs the build path on the last line (e.g., `builds/050dac72b4d8`)
...
The catalog is stored in `~/.config/xorq/` on each CI runner. Catalog entries are **ephemeral** and only exist for the duration of the workflow run. If you need persistent catalog entries across CI runs, use shared storage (e.g., mounted volume, S3) or export/import the catalog.
:::
