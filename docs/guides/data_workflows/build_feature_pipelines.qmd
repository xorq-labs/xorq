...
2. Create three pipeline steps: a scaler to standardize features, a selector to choose the top 2 features, and a classifier for predictions. `params_tuple` passes parameters to the scikit-learn transformers (e.g., `k=2` for SelectKBest, `random_state=42` for LogisticRegression).
...
```
1. Create a `ParquetCache` that persists fitted models on disk. Features survive Python session restarts and can be shared across team members when `base_path` points to shared storage (e.g., `/mnt/shared-storage`, network mount, or S3). The cache directory is created at `base_path / relative_path`.
...
2. The build output is a directory named with a hash (e.g., `a3f8d2c1`) that uniquely identifies this pipeline configuration. The directory contains `expr.yaml` (pipeline structure), cached fitted models, and metadata for reproducibility.
...
Compute and log mean and standard deviation for each feature so you can compare production to training. If production feature distributions differ significantly from training, it may indicate drift or data quality issues.
...
2. Log mean and standard deviation for each feature. These statistics help detect data drift: if production feature distributions differ significantly from training, it may indicate data quality issues or distribution shifts that could affect model performance.
...
```
1. Create a scaler step with a descriptive name. The `name` parameter helps identify this step in logs, monitoring, and debugging. Use names that describe the transformation purpose (e.g., `income_scaler` rather than generic `scaler`).
2. Document the transformation rationale in comments. Explain why this specific transformation is needed (e.g., preventing feature magnitude bias in distance-based algorithms). This documentation helps maintainers understand design decisions when modifying pipelines.
...
```
1. Create separate pipelines for different feature groups (e.g. numeric vs other). Modular pipelines are easier to test, debug, and modify independently. Each pipeline handles transformations specific to its feature type.
2. Fit each pipeline independently on its feature subset. Separate fitting allows you to cache, version, and monitor each feature group separately. Combine the fitted pipelines later (e.g., concatenate transformed features) for the full feature set.
...
```
1. Check for missing columns by computing the set difference between expected and actual columns. Raise an error if any required columns are missing, preventing pipeline failures from schema mismatches.
2. Validate inference data schema before running predictions. Schema validation catches data issues early (e.g., renamed columns, missing features) and provides clear error messages instead of cryptic transformation errors.
