...
```
1. Create a `ParquetCache` that persists fitted models on disk. Features survive Python session restarts and can be shared across team members when `base_path` points to shared storage (e.g., network mount, S3). The cache directory is created at `base_path / relative_path`.
...
```
1. Create separate pipelines for different feature groups (numeric vs categorical). Modular pipelines are easier to test, debug, and modify independently. Each pipeline handles transformations specific to its feature type.
