...
Xorq's `Pipeline.from_instance()` method wraps scikit-learn pipelines, preserving all sklearn functionality while adding deferred execution and integration with Xorq expressions. Your existing sklearn code works unchanged - you just wrap it to gain Xorq's benefits.
...
Wrap your existing sklearn pipelines with `Pipeline.from_instance()` to add deferred execution and Xorq integration. The wrapped pipeline preserves all sklearn functionality while enabling optimization and caching.

**Key points**:

- `Pipeline.from_instance()` accepts any sklearn `Pipeline` object
- You can use `make_pipeline()` or `Pipeline()` constructors - both work
- The wrapped pipeline is immutable (frozen) but supports parameter updates
...
```bash
python wrap_and_predict.py
```

The output is a pandas DataFrame with predictions:
...
- Wrapped pipelines work with Xorq expressions, not pandas DataFrames
- `.fit()` returns a `FittedPipeline` that supports deferred execution
- `.predict()` creates an expression that executes only when you call `.execute()`
- This enables optimization and caching of the entire pipeline

## Manage pipeline parameters

Xorq pipelines support sklearn's parameter naming convention for updating parameters. Use `set_params()` with the `step__parameter` syntax.
...
- Use `step_name__parameter_name` syntax (double underscore)
- `set_params()` returns a new pipeline instance (pipelines are immutable)
- All sklearn parameter names and values are supported
- Parameter updates work the same as in sklearn
...
- Create multiple pipeline variants efficiently
- Each `set_params()` call creates a new pipeline instance
- Use `score_expr()` to evaluate fitted pipelines on test data
- Compare parameter settings without modifying original pipeline
...
- Xorq pipelines produce identical predictions to sklearn
- Use `np.allclose()` for floating-point comparisons
- Scores should match exactly (or within floating-point precision)
- This equivalence enables seamless migration from sklearn to Xorq
...
```bash
python access_sklearn_instance.py
```
...
Pipeline parameters: ['memory', 'steps', 'transform_input']...
```

**Key points**:

- `.instance` property returns the equivalent sklearn pipeline
- Useful for debugging or when you need sklearn-specific features not exposed by Xorq
- The instance reflects current parameter settings
- Changes to the instance don't affect the Xorq pipeline (immutability)
...
```
First fit (computes model)...
First fit completed in 0.055 seconds

Second fit (uses cache)...
Second fit completed in 0.005 seconds
...
- Caching works with wrapped sklearn pipelines
- Fitted models are cached based on content-addressed hashing
- Cache hits avoid recomputation of fitted steps
- See [Train models at scale](./train_models_at_scale.qmd) for detailed caching examples

## Deploy sklearn models

Prepare your wrapped sklearn pipelines for deployment by creating a prediction expression with a placeholder input. Wrapped sklearn pipelines work the same as native Xorq pipelines for deployment.

### Create prediction expression

Create `prepare_deployment.py` to create a prediction expression for serving:
...
- Use a placeholder input table with the same schema as your training features
- Tag the placeholder with `"model_input"` for deployment
- The prediction expression is deferred until execution
- See [Deploy models to production](./deploy_models_to_production.qmd) for complete deployment workflow including server setup and client code
...
**Error**: `set_params()` doesn't change behavior or raises an error

**Cause**: Using wrong parameter name format or parameter doesn't exist
...
4. Use correct parameter value type (e.g., string vs. int)
...
**Error**: Need to access underlying sklearn model for custom operations
...
3. Update sklearn version: ensure compatible sklearn version
4. Test with simple pipeline first: wrap a minimal pipeline to isolate issue
...
- [Deploy models to production](./deploy_models_to_production.qmd) - Build and serve your sklearn models
