...
Start with the simplest evaluation: compute a single metric to get quick feedback on performance. This approach gives you an immediate sense of whether your model learns patterns before you invest time in comprehensive analysis. Use the fitted pipeline's `score_expr()` method, which uses the model's default metric: accuracy for classifiers and R² for regressors.
...
- `score_expr()` computes and returns the default metric value directly
- For classifiers, the default metric is accuracy
- For regressors, the default metric is R²
- The metric is computed immediately when you call `score_expr()`

This script provides a simple way to get a single metric value for quick evaluation.

## Calculate multiple metrics

Basic metrics give you a single number, but production evaluation requires multiple metrics to understand model behavior comprehensively. A single metric, such as accuracy, doesn't reveal precision-recall trade-offs or class-specific performance. Get predictions and compute various metrics directly using scikit-learn.
...
- This approach works with all scikit-learn metric functions
...
Run this code after the previous example to see:
...
Single-model evaluation shows performance, but model selection requires comparing multiple candidates to find the best fit for your data. Different algorithms have different strengths, and you can't know which performs best without a systematic comparison. Evaluate several models systematically to identify the best performer for your data.
...
# In production, write to database or experiment tracking system
...
You need to access the fitted model directly to get probability predictions:
```python
from sklearn.metrics import roc_auc_score
