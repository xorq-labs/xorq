...
- **Python script**: Copy code blocks into a `.py` file and run with `cache_expr.py`
...
The `.cache()` method tells Xorq to store results from this expression. On the first run, Xorq computes and caches the results. On the next execution of the same expression, it retrieves them directly from cache.
