...
iris = xo.examples.iris.fetch(backend=con)  # <2>

filtered = iris.filter(xo._.sepal_length > 5)  # <3>

grouped = filtered.group_by("species").agg(   # <4>
    xo._.sepal_width.sum()
)

result = grouped.execute()   # <5>
print(result)
```
...
- [Defer query execution](defer_query_execution.qmd) — Learn when Xorq builds expressions versus when it runs computation
- [Cache expression results](explore_caching.qmd) — Understand cache hits and misses, plus how invalidation works
