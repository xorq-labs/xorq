...
If you run into issues during installation or setup, here are solutions to common problems.

### Installation fails with "No matching distribution found"

This usually means you're using an older Python version. Xorq requires Python 3.10 or higher.

**Solution:** Verify your Python version and upgrade if needed:

```bash
python --version  # Should show 3.10 or higher
```

If you need to upgrade, visit the [Python downloads page](https://www.python.org/downloads/).

### ModuleNotFoundError when importing Xorq

This happens when Xorq isn't installed in your current Python environment.

**Solution:** Make sure you're in the correct virtual environment and reinstall:

```bash
pip install xorq
```

If you're using virtual environments, activate the correct one first:
...
Connection failures typically stem from missing credentials or incorrect configuration.
...
If they're empty, set them before running your code.

**For Snowflake/Trino:**

Verify your credentials and network access. Firewalls or VPNs might block connections to remote databases.

### "No module named 'duckdb'" error

This means you haven't installed the backend you're trying to use.

**Solution:** Install the specific backend:
...
### Query execution is slow or hangs

This can happen with large datasets or complex queries on underpowered backends.

**Solution:** Try switching to a more appropriate backend for your workload:

- Use DuckDB for analytical queries on moderate-sized data
- Use Pandas for small datasets and quick prototyping
- Use Snowflake or BigQuery for large-scale production workloads

### Still having issues?

If these solutions don't help, check the [GitHub issues](https://github.com/xorq-labs/xorq/issues) or join the [Discord community](https://discord.gg/8Kma9DhcJG) for support.
