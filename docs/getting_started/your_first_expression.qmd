...
::: {.callout-note}
### Prerequisites
Before starting, make sure you have Xorq installed. See [Install Xorq](../how_to/install_xorq.qmd) if you need help.
:::
...
3. Preview the first five rows to see the data structure.
...
2. Filter rows where sepal length is greater than five.
...
- [Understand deferred execution](understand_deferred_execution.qmd) explains when computation happens versus when expressions are built
- [Explore caching](explore_caching.qmd) shows you how to speed up repeated queries
- [Switch backends](switch_backends.qmd) teaches you how to run the same expression on different engines
