...
build an a pipeline that works with the [Palmer Penguins Dataset](https://www.kaggle.com/datasets/parulpandey/palmer-archipelago-antarctica-penguin-data) to predict an observed penguin's species given it's duck bill length and depth.  Then you will serve this prection model up for consumption by other services.
...
Create a new Xorq project using the built-in penguins template. This template demonstrates a complete ML workflow wit
h the [Palmer Penguins dataset](https://www.kaggle.com/datasets/parulpandey/palmer-archipelago-antarctica-penguin-data):
```bash
...
Here we will show what it looks like to use sklearn inside of a xorq pipeline.
...
Execute your built pipeline and view the results. Replace `<BUILD_HASH>` with the hash from your build output in step 3: You will see a ...?
...
Serving unbound allows you to ???  The unbound hash is later used to ??
xorq serve-unbound builds/$BUILD_HASH --host localhost --port 8001 --to_unbind_tag test

serving unbound allows you to unspecify a node and let that be passed in.  In this case , we are unspecifying the test node.  Now when data is passed in in the form of bill_length_mm, bill_depth_mmm, then the computations will flow through and a prediction will be returned.
...
Her we use a UDXF expression to perform arbitrary pandas transforms on nodes and serve them. 
