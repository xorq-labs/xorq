...
**Hash computation**: Xorq computes a content hash from the expression logic. This hash becomes the build directory name and serves as the unique identifier.
...
The build process is deterministic. Building the same expression twice produces the same hash and identical artifacts. This enables reproducible deployments.
...
graph TD
    ...
    A -->|Yes| B[Build required]
    A -->|No| C{Need reproducibility?}
    C -->|Yes| B
    C -->|No| D{Multiple environments?}
    D -->|Yes| B
    D -->|No| E{Interactive development?}
    E -->|Yes| F[Run directly]
    E -->|No| B
```
