...
**Execution**: You reference catalog entries by alias in commands like `xorq run customer-features` or `xorq serve-unbound fraud-model`. The catalog resolves the alias to the current build hash. The catalog registration and lookup process works like this:
...
The catalog doesn't store builds; it indexes them. Builds live in the `builds/` directory. The catalog maintains aliases that point to those builds. The workflow from registration to execution follows this path:
