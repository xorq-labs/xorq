...
<style>
:root {
  --xorq-midnight: 186 68% 6%;
  --xorq-blueSky: 195 100% 88%;
  --xorq-cloud: 196 100% 97%;
  --xorq-teal: 186 68% 30%;
  --xorq-accent: 186 68% 30%;
  --xorq-code-fg: 186 68% 6%;
  --xorq-code-bg-alpha: 0.25;
  --xorq-mono: "FK Grotesk Mono", "Space Mono", ui-monospace, SFMono-Regular, Menlo, monospace;
}

body.quarto-dark {
  --xorq-accent: 195 100% 85%;
  --xorq-code-fg: 196 100% 97%;
  --xorq-code-bg-alpha: 0.15;
}

main h2 {
  border-left: 4px solid hsl(var(--xorq-blueSky));
...
main h5::before,
main h5::after,
body.quarto-dark main h3::before,
body.quarto-dark main h3::after,
body.quarto-dark main h4::before,
body.quarto-dark main h4::after,
body.quarto-dark main h5::before,
body.quarto-dark main h5::after {
  content: "" !important;
  display: none !important;
}

main table {
  font-family: var(--xorq-mono);
...
main table thead th {
  ...
  font-weight: 500;
  color: hsl(var(--xorq-accent));
  border-bottom: 2px solid hsl(var(--xorq-blueSky));
}

main :not(pre) > code {
  font-family: var(--xorq-mono);
  color: hsl(var(--xorq-code-fg));
  background: hsl(var(--xorq-blueSky) / var(--xorq-code-bg-alpha));
  border-radius: 0;
  padding: 0.1em 0.35em;
}

main :is(h1, h2, h3, h4, h5, h6) code {
...
A **cache key** is a hex string that uniquely identifies a cached result. It's computed by hashing the expression via [dask tokenization](https://docs.dask.org/en/stable/custom-collections.html#tokenize). The key is prefixed with `letsql_cache-` by default.
...
The key is stable even if the underlying data changes. This is useful when you only need valid data and don't care about updates. Only high-level information like path, table name, and schema matter.
...
Stores parquet files in a GCS bucket instead of local disk. Same pattern as `ParquetStorage`, but reads and writes go through `gcsfs`.
...
- **`CachedNode`**: the internal DAG node created by `.cache()`. It wraps the uncached expression and a reference to the `Cache` object. During execution, it's replaced with the cached result.
...
- **`RemoteTable`**: the DAG node representing data transferred between backends.
- **Record batch**: Arrow's columnar data chunk. A `RecordBatchReader` streams these one at a time.
- **ADBC**: Arrow Database Connectivity. The protocol Postgres, Snowflake, and SQLite use to load Arrow data.
