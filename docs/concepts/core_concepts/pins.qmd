...
Xorq wraps the [pins Python library](https://rstudio.github.io/pins-python/) with a preconfigured connection to a public GCS bucket (`letsql-pins`). No authentication is required.
...
Downloads the named pin and returns a local path. Extra keyword arguments
(such as `version`) are forwarded to the underlying pins library via
`board.pin_download`.
...
- [The catalog as executable memory](/concepts/understanding_xorq/catalog_as_executable_memory.qmd)—the catalog versions whole expressions the way pins version data artifacts.
- [Caching](/concepts/understanding_xorq/intelligent_caching_system.qmd)—how Xorq decouples stored results from their source.
