...
rule moves build hashes and leaves cache keys alone. That's the case below,
and it's the common one for an upgrade.

It's not a blanket guarantee: cache keys are computed *by* those rules, so an
...
- **Cache keys don't change.** Existing cache entries still hit, and nothing
  recomputes.
