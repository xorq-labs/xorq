...
Build hashes and cache keys are separate, and only the first is affected. Cache
keys are computed by the [cache strategies](intelligent_caching_system.qmd) and
are deliberately rule-set-neutral, so an upgrade that moves build hashes does
not move cache keys.
