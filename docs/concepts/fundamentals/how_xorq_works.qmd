...
```{mermaid}
graph TB
    User[User Code] --> ExpGraph[Expression Graph]
    ExpGraph --> Compiler[Compiler]
    Compiler --> Manifest[YAML Manifest]
    Manifest --> Catalog[Catalog]
    Catalog --> Executor[Executor]
    Executor --> Backend[Backend Engine]
    Backend --> Cache[Cache System]
...
Here's the key insight: The expression graph is an intermediate representation (IR) that's independent of any specific engine. This IR is what makes multi-engine execution possible.
```{mermaid}
sequenceDiagram
    participant User
    participant Xorq
...
If you're new to Xorq, start with [Overview](overview.qmd) to learn about the high-level architecture and components. [Why deferred execution](why_deferred_execution.qmd) explains how lazy evaluation works in Xorq.

[Expression format](expression_format.qmd) covers the detailed specifications of YAML manifest structure. [Build system](../reproducibility/build_system.qmd) explains how `xorq build` works internally. [Compute catalog](../reproducibility/compute_catalog.qmd) details how the catalog supports discovery and reuse across teams.

[Content-addressed hashing](../reproducibility/content_addressed_hashing.qmd) explains how Xorq generates content hashes. [Multi-engine execution](../execution_and_backends/multi_engine_execution.qmd) covers how one manifest runs on multiple backends.
