---
title: 'Core Concepts'
description: "The core concepts to understand what's Xorq"
---
...
train_udaf = udf.agg.pandas_df(
    fn=lambda df: pickle.dumps(
        xgb.train({"objective": "reg:squarederror"}, xgb.DMatrix(df[list(features)], df[target]), num_boost_round=10)
    ),
...
predict_udf = make_pandas_expr_udf(
    computed_kwargs_expr=train_udaf.on_expr(t),
    fn=lambda model, df: model.predict(xgb.DMatrix(df[list(features)])),
    schema=t[features].schema(),
    return_type=dt.float32,
    name="pred",
)
expr = t.mutate(predict_udf.on_expr(t).name("pred"))
```
