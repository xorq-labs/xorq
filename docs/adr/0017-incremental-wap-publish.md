...
Build one canonical `MERGE` directly on the sqlglot AST — the builder API (`exp.merge` / `exp.When`,
identifiers `quoted=True`), no string assembly and no hand-rolled quoter — and hand to `raw_sql`,
which renders the backend dialect via `.sql(dialect=con.dialect)`. Verified to transpile cleanly to
all five dialects; gizmosql is a DuckDB-backed Flight SQL server (`compiler = sc.duckdb.compiler`),
so it renders through the duckdb dialect and adds no sixth dialect to verify.
...
def _merge_query(final, staging, key, columns, mode):
    ...
    insert = exp.Insert(this=exp.tuple_(*(_col(c) for c in [*key, *data])),
                        expression=exp.tuple_(*(_col(c, "s") for c in [*key, *data])))
    whens = []
    if mode is PublishMode.MERGE:
        # _not_del() is exp.paren((s._op <> 'D' OR s._op IS NULL)) — explicit parens, since the
        # condition lands after the implicit AND in `WHEN MATCHED AND <cond>`.
        whens.append(exp.When(matched=True, condition=_col("_op", "s").eq("D"), then=exp.Delete()))
...
def _publish_statement_dml(con, staging, final, key, columns, mode):
    import sqlglot.expressions as exp
    data  = [c for c in columns if c not in key and c != "_op"]
    merge = mode is PublishMode.MERGE          # NULL _op upserts (only literal 'D' deletes)

    def onf():   # the UPDATE target can't be aliased on this tier: qualify by table name
    ...
    if data:                                   # key-only changeset: nothing to UPDATE
        stmts.append(exp.Update(
            ...
            where=exp.Where(this=exp.and_(onf(), _not_del()) if merge else onf())))
    absent = exp.Exists(this=exp.select("1").from_(_tbl(final)).where(onf())).not_()
    stmts.append(_insert_select(final, staging, [*key, *data], alias="s",
                                where=exp.and_(absent, _not_del()) if merge else absent))
    if merge:
        stmts.append(exp.delete(_tbl(final), where=exp.Exists(
            this=exp.select("1").from_(_tbl(staging, "s"))
...
which renders (sqlite dialect, `MERGE`, `key=["id"]`, one data column `b`) as:

```sql
UPDATE "final" SET "b" = "s"."b" FROM "staging" AS s
  WHERE "final"."id" = "s"."id" AND ("s"."_op" <> 'D' OR "s"."_op" IS NULL);
INSERT INTO "final" ("id", "b") SELECT "id", "b" FROM "staging" AS s
  WHERE NOT EXISTS(SELECT 1 FROM "final" WHERE "final"."id" = "s"."id")
    AND ("s"."_op" <> 'D' OR "s"."_op" IS NULL);
DELETE FROM "final" WHERE EXISTS(
  SELECT 1 FROM "staging" AS s WHERE "final"."id" = "s"."id" AND "s"."_op" = 'D');
```
...
and only then with the identity/trigger warning. Statements are built on the sqlglot AST with the
same helpers as Tier 1 (`_col`/`_tbl`/`_not_del`/`_insert_select`) and rendered per dialect via
