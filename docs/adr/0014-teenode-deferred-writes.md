...
- **Transport** is *how* batches reach the target: the client-side Arrow generator (Phase 1) or an
...
concretes are `@frozen` attrs classes (immutable, value-equal, with converters/validators: for
example `mode` coerces from a string, `con` is validated to expose `read_record_batches`). Immutability and
...
- **Append / pass-through**: a data-modifying CTE,
  `WITH w AS (INSERT INTO target SELECT … RETURNING *) SELECT * FROM w`, which writes as a side effect
...
- **The write is not all-or-nothing for backends.** `BackendWriteThrough`'s per-batch path
  (mode-capable backends, for example Postgres/ADBC) commits each batch individually, so a mid-stream
  ...
  consumed data the write-through never persisted, so the "write-then-yield" guarantee is inverted.
