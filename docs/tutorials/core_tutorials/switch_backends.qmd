---
title: 'Switch backends'
description: "Run the same expression on DuckDB, Pandas, DataFusion, and more"
icon: "arrow-left-right"
headline: "Master multi-engine execution"
---
...
By the end, you'll know how to pick the right backend for your workload.
...
Let's start with Xorq's default embedded backend. This uses a modified DataFusion engine optimized for Arrow operations.
...
Now let's run the same expression on DuckDB. DuckDB excels at analytical queries and works well with larger datasets.
...
Pandas is great for small datasets and interactive analysis. Let's run the expression there.
...
Let's time the same query on different backends to see performance characteristics.
...
- [Optimize pipeline performance](/guides/performance_workflows/optimize_pipeline_performance.qmd) covers backend selection strategies
- [Switch backends dynamically](/guides/performance_workflows/switch_backends_dynamically.qmd) teaches advanced backend-switching patterns
