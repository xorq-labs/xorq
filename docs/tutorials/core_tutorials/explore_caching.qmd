...
description: "See cache hits and misses in real time and speed up your workflows"
icon: "lightning-charge"
headline: "Leverage intelligent caching"
---
...
By the end, you'll know how to use caching to speed up your workflows.
...
This is especially powerful for expensive operations like:
...
Let's start by connecting to a backend and setting up a cache storage location.
...
```
1. Connect to the embedded backend where cached data will be stored.
2. Create a SourceStorage object that manages the cache.

`SourceStorage` stores cached results in your backend as tables. When you run an expression with `.cache()`, Xorq saves the results and reuses them on subsequent runs.

## Cache your first expression

Now let's build an expression and add caching to it.
...
3. Add caching with `.cache(storage)`.

The `.cache()` method tells Xorq to store results from this expression. On the first run, Xorq will compute and cache. On subsequent runs, it'll fetch from cache.

## Observe cache miss (first run)

Let's execute the expression for the first time. This will be a cache miss—Xorq has to compute the results.
...
2. Execute the expression—this triggers computation and caching.
...
Now let's run the same expression again. This time you'll see a cache hit.
...
:::{.callout-note}
### Cache key
Xorq computes a hash from your expression's structure and data sources. If the expression is identical, the hash matches, and you get a cache hit.
:::

## Understand cache invalidation

What happens if you change the expression? Let's modify the filter and see cache invalidation in action.
...
Let's run several executions and see the timing difference between cache hits and misses.
...
:::{.callout-warning}
### Cache storage
SourceStorage keeps cached data in your backend as tables. Make sure you have enough storage space for cached results, especially with large datasets.
:::
...
```{python}
# <1>
step1 = iris.filter(xo._.sepal_length > 5).cache(storage)
