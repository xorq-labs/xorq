...
::: {.callout-note}
### Running Python in the project venv
`uv add` set up a `.venv` in `flights-tutorial/`; every Python invocation in this tutorial needs to use it. Two equivalent options:

- **Recommended:** prefix each `python` command with `uv run` — e.g. `uv run python flights_catalog.py`. uv finds the project venv from `pyproject.toml` and uses it; no activation step.
- **Or:** activate once per shell with `source .venv/bin/activate`, then call plain `python flights_catalog.py`. Run `deactivate` to leave the venv when you're done.

The rest of the tutorial uses the `uv run python ...` form.
:::
...
::: {.callout-tip}
### Run as you go
`flights_catalog.py` grows section by section through the rest of the tutorial. After each addition, run `uv run python flights_catalog.py` from your project directory — the output shown beneath each block (dimensions, measures, query results) is what you'll see when you do. The complete script is consolidated at the end in [Putting it all together](#putting-it-all-together).
:::
...
```bash
uv run python flights_catalog.py
```
