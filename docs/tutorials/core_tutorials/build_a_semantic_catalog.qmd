...
- Xorq installed with BSL and DuckDB: `pip install "xorq[bsl,duckdb]"`
...
::: {.callout-note}
### Row order
BSL doesn't sort the result — the row order you see depends on the backend's hash layout. Add `.order_by("origin")` before `.execute()` if you want a stable order.
:::
...
```python
model.query(dimensions=("airport",), measures=("flight_count",)).execute()
```
...
To preserve the model itself — not just one query result — turn it into a Xorq-tagged expression with `to_tagged(model)`, then add it to the catalog. The catalog is git-backed, so the directory you point it at becomes a versioned store of every entry you add.
...
# <3>
catalog.add(tagged_model, aliases=("flights-model",), sync=False)

print("Catalog at:", catalog_dir)
print("Aliases:   ", catalog.list_aliases())
```
1. `to_tagged(model)` serializes the dimensions, measures, and underlying table into a Xorq expression with BSL metadata attached. You're cataloging *the model*, not the result of one of its queries.
2. `Catalog.from_repo_path(..., init=True)` initializes a fresh git repo at that path. For a shared catalog, use `Catalog.from_name("my-catalog")` or `Catalog.clone_from("git@...")` instead.
...
Here's the payoff. Pull the entry back out of the catalog and rebuild the original `SemanticModel` with `from_tagged`:
...
```
1. `flights_entry` is the catalog handle — content hash, alias, sidecar metadata, and the cataloged expression on `.expr`.
2. `from_tagged(...)` reads the BSL metadata off the cataloged expression and reconstructs a live `SemanticModel`: same dimensions, same measures, same underlying table.
...
This is the property that makes the catalog interesting: you stored the model, and now any consumer can ask anything the model's dimensions and measures are designed to answer.
...
# 5. Recover the model and run a new query
flights_entry = catalog.get_catalog_entry("flights-model", maybe_alias=True)
flights_model = from_tagged(flights_entry.expr)
...
- `to_tagged(model)` produces a catalog-ready expression, and `from_tagged(entry.expr)` reconstructs the live `SemanticModel` on the other side.
