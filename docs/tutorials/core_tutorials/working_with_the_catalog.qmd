...
Recreate the catalog at a stable path (`~/work/flights-catalog`) so the rest of the tutorial has somewhere persistent to point at, then re-add the flights model the same way the foundation tutorial did. From inside the `flights-tutorial/` project:
