...
Xorq provides a caching mechanism that can write to Parquet to avoid re-computation:
...
def standardize_penguins(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    clean = df.dropna(subset=["bill_length_mm", "bill_depth_mm", "body_mass_g"]).copy()

    clean["bill_length_std"] = (
        (clean["bill_length_mm"] - clean["bill_length_mm"].mean()) 
        / clean["bill_length_mm"].std()
    )

    clean["body_mass_std"] = (
        (clean["body_mass_g"] - clean["body_mass_g"].mean()) 
        / clean["body_mass_g"].std()
    )
...
flight_udxf(
    process_df=standarized_penguins,
    maybe_schema_in=penguins.schema()
)

# Chain UDXF nodes
pipeline_result = (
    penguins
    .pipe(standardize_penguins)
    .execute()
)
```
...
pandas_profile.save("pandas_example")
```
