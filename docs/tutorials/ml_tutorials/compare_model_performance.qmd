...
- Basic familiarity with [scikit-learn classifiers](https://scikit-learn.org/stable/supervised_learning.html#supervised-learning)
- Understanding of [train/test splits](https://scikit-learn.org/stable/modules/cross_validation.html#cross-validation)
...
This tutorial shows you how to train three models (k-nearest neighbors, decision tree, random forest) and compare their accuracy scores.
...
Train a single classifier and measure its accuracy. Think of it this way: you're establishing a baseline before comparing multiple models.
...
Here's where the pattern shines. You define your models, wrap each in a pipeline, and then evaluate them all:
...
Here's the full workflow in one place. If you started with the Python shell, then you've already run all of this. If you want to create a script, then here's everything combined:
...
- [Train your first model](train_your_first_model.qmd) — Learn the basics of model training with Xorq
- [Split data for training](split_data_for_training.qmd) — Create proper train/test/validation splits
