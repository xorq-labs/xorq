...
- Familiarity with Xorq expressions (see [Defer query execution](understand_deferred_execution) tutorial)
...
UDFs work in SQL queries without any extra configuration. This example registers your table and calls the `calculate_age` UDF in both SELECT and WHERE clauses:
...
- [Train your first model](train_your_first_model.qmd) shows complete ML training workflows
- [Split data for training](split_data_for_training.qmd) demonstrates data preparation patterns
- [Compare model performance](compare_model_performance.qmd) covers systematic model evaluation
