...
def _reader(obj: pa.RecordBatchReader) -> pa.RecordBatchReader:
    ...
    try:
        ...
        # metadata swap would not. Which Arrow error that surfaces as depends
        # on where the cast gives up and has moved between releases, so catch
        # the common base rather than a list: the target schema is the
        # reader's own with one metadata key removed, so any failure here is a
        # pyarrow limitation and never a real schema mismatch. Wrapping has no
        # such constraint: a carrying reader comes back as a new one draining
        # the original, which the wrapper then owns -- errors surface from it
        # as it is consumed, and discarding it unconsumed leaves the original
        # unread.
