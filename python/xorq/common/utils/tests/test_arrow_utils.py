...
def test_drop_dictionary_reader() -> None:
    """A dictionary column may lack a self-cast kernel in some Arrow releases."""
...
@pytest.mark.parametrize(
    "error",
    # What Arrow raises depends on where the cast gives up: no kernel at all,
    # versus ``CastingRecordBatchReader::Make`` refusing a field pair ``CanCast``
    # rejects, which surfaces as ``ArrowTypeError``. The handler catches the
    # shared ``ArrowException`` base so a release that moves the boundary again
    # still falls back instead of propagating.
    [
        pytest.param(pa.ArrowNotImplementedError, id="not-implemented"),
        pytest.param(pa.ArrowTypeError, id="type-error"),
        pytest.param(pa.ArrowInvalid, id="invalid"),
        pytest.param(pa.ArrowException, id="base"),
    ],
)
def test_drop_reader_falls_back_when_cast_is_unsupported(
    pandas_sourced_table: pa.Table, error: type[Exception]
) -> None:
    class NoCastReader:
        ...
        def cast(self, schema):
            raise error("no cast kernel")

    handler = drop_pandas_schema_metadata.registry[pa.RecordBatchReader]
    dropped = handler(NoCastReader())
    assert dropped.schema.metadata is None
    # Per batch, not just the declared schema: ``read_all`` compares batch
    # schemas with ``check_metadata=false``, so it would accept batches that
    # still carry the blob and disagree with what the wrapper declares.
    batches = list(dropped)
    assert batches and all(batch.schema.metadata is None for batch in batches)
    assert (
        pa.Table.from_batches(batches).to_pydict() == pandas_sourced_table.to_pydict()
    )
