...
# regex strips everything up to and including the last ``xorq-catalog-<…>/``
...
def _extract_duckdb_file_paths(sql_ddl: str) -> tuple[str, ...]:
    ...
    # ReadParquet stores the path in expressions[0] (not func.this like ReadCSV).
    # Restrict to the first expression to avoid capturing string-valued kwargs.
    parquet_paths = tuple(
        canon(lit.this)
