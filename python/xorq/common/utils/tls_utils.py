...
def create_ca_keypair(
    ca_cert_file: str | Path,
    ca_key_file: str | Path,
    overwrite: bool = True,
...
@frozen
class BaseKeyPair:
    cert_file = field(validator=instance_of(Path))
    key_file = field(validator=instance_of(Path))
    maybe_tmpdir = field(
        validator=optional(instance_of(TemporaryDirectory)), default=None
    )

    def __attrs_post_init__(self):
        assert self.cert_file.exists() and self.key_file.exists()

    def cleanup(self):
        if self.maybe_tmpdir:
            self.maybe_tmpdir.cleanup()


@frozen
class TLSKeyPair(BaseKeyPair):
    @property
    def cert_chain(self):
        return self.cert_file.read_bytes()
    ...
    @property
    def tls_certificates_paths(self):
        # for FlightServer
        return [(self.cert_file, self.key_file)]

    @property
    def tls_certificates(self):
        return [(self.cert_chain, self.private_key)]

    @classmethod
    def from_common_name(cls, common_name=socket.gethostname()):
        tmpdir = TemporaryDirectory()
        tls_dir = Path(tmpdir.name)
        cert_file = tls_dir.resolve().joinpath("server.crt")
        key_file = tls_dir.resolve().joinpath("server.key")
        create_tls_keypair(cert_file, key_file, common_name=common_name)
        return cls(cert_file, key_file, tmpdir)

    @classmethod
    def from_key_and_cert_paths(cls, key, cert):
        cert = Path(cert)
        key = Path(key)
        return cls(cert, key, None)


@frozen
class CAKeyPair(BaseKeyPair):
    @property
    def root_certificates(self):
        return self.cert_file.read_bytes()

    @classmethod
    def from_common_name(cls, common_name="flight_server_ca"):
        tmpdir = TemporaryDirectory()
        tls_dir = Path(tmpdir.name)
        cert_file = tls_dir.resolve().joinpath("server_ca.crt")
        key_file = tls_dir.resolve().joinpath("server_ca.key")
        create_ca_keypair(cert_file, key_file, ca_common_name=common_name)
        return cls(cert_file, key_file, tmpdir)


@frozen
class ClientKeyPair(BaseKeyPair):
    @property
    def mtls(self):
        return self.cert_file, self.key_file

    @classmethod
    def from_ca_key_pair(cls, key_pair: "CAKeyPair", common_name="flight_client"):
        tmpdir = TemporaryDirectory()
        tls_dir = Path(tmpdir.name)
        cert_file = tls_dir.resolve().joinpath("client.crt")
        key_file = tls_dir.resolve().joinpath("client.key")
