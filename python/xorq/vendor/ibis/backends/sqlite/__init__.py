...
from xorq.vendor.ibis.backends import UrlFromPath
from xorq.vendor.ibis.backends.sql import SQLBackend
from xorq.vendor.ibis.backends.sql.compilers.base import C, F
from xorq.vendor.ibis.backends.sqlite.converter import SQLitePandasData
from xorq.vendor.ibis.backends.sqlite.udf import ignore_nulls, register_all
...
class Backend(SQLBackend, UrlFromPath):
    ...
    def _inspect_schema(
        ...
        rows = cur.fetchall()
        if not rows:
            raise com.XorqError(f"Table not found: {table_name!r}")
        ...
        if unknown:
            ...
            if row is not None:
                ...
                raise com.XorqError(f"Failed to infer types for columns {unknown}")
