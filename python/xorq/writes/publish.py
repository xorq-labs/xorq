...
def _col(name: str, table: str | None = None):
    """Quoted column reference, optionally table-qualified."""
    import sqlglot.expressions as exp  # noqa: PLC0415

    return exp.column(name, table=table, quoted=True)


def _tbl(name: str, alias: str | None = None):
    """Quoted table reference, optionally aliased."""
    import sqlglot.expressions as exp  # noqa: PLC0415

    return exp.table_(name, alias=alias, quoted=True)


def _not_del():
    """``(s."_op" <> 'D' OR s."_op" IS NULL)`` — 'D' deletes, anything else upserts.

    Parenthesized explicitly: the node lands after an implicit ``AND`` in MERGE
    ``WHEN`` conditions, where sqlglot has no parent to force precedence parens.
    """
    import sqlglot.expressions as exp  # noqa: PLC0415

    return exp.paren(_col("_op", "s").neq("D").or_(_col("_op", "s").is_(exp.null())))


def _insert_select(final, staging, cols, alias=None, where=None):
    """``INSERT INTO final (cols) SELECT cols FROM staging [AS alias] [WHERE …]``."""
    import sqlglot.expressions as exp  # noqa: PLC0415

    select = exp.select(*(_col(c) for c in cols)).from_(_tbl(staging, alias))
    if where is not None:
        select = select.where(where)
    return exp.insert(
        select, _tbl(final), columns=[exp.to_identifier(c, quoted=True) for c in cols]
    )
...
def _merge_query(
    ...
    """A dialect-agnostic ``MERGE``, built on the sqlglot AST (no string assembly).

    ``.sql(dialect=con.dialect)`` renders it per backend; identifier quoting and
    escaping come from the AST (``quoted=True``), not hand-rolled.
    """
    import sqlglot.expressions as exp  # noqa: PLC0415

    data = [c for c in columns if c not in key and c != "_op"]
    on = exp.and_(*(_col(k, "f").eq(_col(k, "s")) for k in key))
    update = exp.Update(expressions=[_col(c).eq(_col(c, "s")) for c in data])
    insert = exp.Insert(
        this=exp.tuple_(*(_col(c) for c in [*key, *data])),
        expression=exp.tuple_(*(_col(c, "s") for c in [*key, *data])),
    )
    whens = []
    if mode is PublishMode.MERGE:
        ...
        whens.append(
            exp.When(
                matched=True, condition=_col("_op", "s").eq("D"), then=exp.Delete()
            )
        )
        if data:  # key-only table: nothing to UPDATE, omit the clause (else empty SET)
            whens.append(exp.When(matched=True, condition=_not_del(), then=update))
        whens.append(exp.When(matched=False, condition=_not_del(), then=insert))
    else:
        if data:  # key-only table: nothing to UPDATE
            whens.append(exp.When(matched=True, then=update))
        whens.append(exp.When(matched=False, then=insert))
    return exp.merge(*whens, into=_tbl(final, "f"), using=_tbl(staging, "s"), on=on)
...
def _publish_native_merge(con, staging, final, key, columns, mode) -> None:
    ...
    APPEND skips the merge for a plain ``INSERT … SELECT``. Both statements are
    built once on the sqlglot AST, then rendered to the backend's dialect via
    ``.sql(dialect=con.dialect)`` — so identifiers transpile correctly (e.g.
    backticks on databricks) and ``raw_sql`` gets a plain string.
    """
    _ensure_final(con, final, staging, columns, mode)
    if mode is PublishMode.APPEND:
        query = _insert_select(final, staging, _data_cols(columns, mode))
    else:
...
def _publish_statement_dml(con, staging, final, key, columns, mode) -> None:
    ...
    built on the sqlglot AST like the merge tier and rendered per dialect.
    """
    import sqlglot.expressions as exp  # noqa: PLC0415

    _ensure_final(con, final, staging, columns, mode)
    if mode is PublishMode.APPEND:
        stmts = [_insert_select(final, staging, _data_cols(columns, mode))]
    else:
        data = [c for c in columns if c not in key and c != "_op"]
        merge = mode is PublishMode.MERGE  # NULL _op upserts (only 'D' deletes)

        def onf():  # the UPDATE target can't be aliased on this tier: qualify by name
            return exp.and_(*(_col(k, final).eq(_col(k, "s")) for k in key))
        ...
        if data:  # nothing to UPDATE for a key-only table
            stmts.append(
                exp.Update(
                    this=_tbl(final),
                    expressions=[_col(c).eq(_col(c, "s")) for c in data],
                    from_=exp.From(this=_tbl(staging, "s")),
                    where=exp.Where(
                        this=exp.and_(onf(), _not_del()) if merge else onf()
                    ),
                )
            )
        absent = exp.Exists(this=exp.select("1").from_(_tbl(final)).where(onf())).not_()
        stmts.append(
            _insert_select(
                final,
                staging,
                [*key, *data],
                alias="s",
                where=exp.and_(absent, _not_del()) if merge else absent,
            )
        )
        if merge:
            stmts.append(
                exp.delete(
                    ...
                    where=exp.Exists(
                        this=exp.select("1")
                        .from_(_tbl(staging, "s"))
                        .where(exp.and_(onf(), _col("_op", "s").eq("D")))
                    ),
                )
            )
    with con.begin() as cur:
        for stmt in stmts:
            cur.execute(stmt.sql(dialect=con.dialect))
    _drop_staging(con, staging)
