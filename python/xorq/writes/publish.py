...
from xorq.writes.enums import BackendType, PublishMode, PublishStrategy
...
# BackendType -> publish strategy. Routing matches a con's *own* already-loaded
# MRO against these type-paths, so picking the strategy for one backend never
# imports another — a minimal install with only duckdb must not need pyiceberg/
# snowflake/etc. importable just to route duckdb. (lazy_singledispatch can't do
# this: it buckets lazy registrations by top-level package and imports the whole
# "xorq.*" bucket on first dispatch, which crashes single-backend installs.)
# postgres is version-aware, so it is handled in _strategy, not this table.
_STRATEGY_BY_BACKEND = {
    ...
    BackendType.GIZMOSQL: PublishStrategy.NATIVE_MERGE,
    BackendType.SQLITE: PublishStrategy.STATEMENT_DML,
    BackendType.PYICEBERG: PublishStrategy.UPSERT_DELETE,
}


def _strategy(con: "BaseBackend", mode: PublishMode) -> PublishStrategy:
    """Route ``con`` to its publish mechanism by its own type, importing nothing.

    Walks ``type(con).__mro__`` (classes already loaded alongside the con) and
    matches fully-qualified names against ``_STRATEGY_BY_BACKEND`` — so routing
    duckdb never imports pyiceberg, which keeps single-backend installs working.
    An unregistered backend (datafusion, xorq_datafusion, pandas) falls back to
    the full-rewrite path.
    """
    for klass in type(con).__mro__:
        path = f"{klass.__module__}.{klass.__qualname__}"
        if path == BackendType.POSTGRES:
            # MERGE INTO needs postgres 15+; con.version is a dotted string like
            # "15.4", so the major version is the part before the first dot.
            return (
                PublishStrategy.NATIVE_MERGE
                if int(con.version.split(".")[0]) >= 15
...
def resolve_strategy(con: "BaseBackend", mode: PublishMode) -> PublishStrategy:
    ...
    before dispatch. Otherwise the strategy is dispatched on the backend type,
    falling back to ``REWRITE`` for any backend not registered above.
    """
    if mode is PublishMode.APPEND:
        return PublishStrategy.APPEND
    return _strategy(con, mode)
...
def _merge_query(
    ...
    """A dialect-agnostic ``MERGE`` statement; ``raw_sql`` renders it per backend."""
    ...
    if mode is PublishMode.MERGE:
        ...
        not_del = """(s."_op" <> 'D' OR s."_op" IS NULL)"""
        parts.append("""WHEN MATCHED AND s."_op" = 'D' THEN DELETE""")
        if data:  # key-only table: nothing to UPDATE, omit the clause (else empty SET)
            parts.append(f"WHEN MATCHED AND {not_del} THEN UPDATE SET {sets}")
        parts.append(
            f"WHEN NOT MATCHED AND {not_del} THEN INSERT ({cols}) VALUES ({vals})"
        )
    else:
...
def _publish_statement_dml(con, staging, final, key, columns, mode) -> None:
    ...
    ``UPDATE ... FROM`` is postgres-native and sqlite >= 3.33.
    """
    _ensure_final(con, final, staging, columns, mode)
    if mode is PublishMode.APPEND:
        cols = ", ".join(_q(c) for c in _data_cols(columns, mode))
...
def _publish_append(con, staging, final, key, columns, mode) -> None:
    ...
    """
    _PUBLISH[_strategy(con, mode)](con, staging, final, key, columns, mode)
