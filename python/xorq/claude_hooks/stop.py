#!/usr/bin/env python3
"""Stop hook for Claude Code - checks for uncataloged builds."""
...
def main():
    ...
    try:
        ...
        if uncataloged:
            ...
            message += (
                f"Found {len(uncataloged)} uncataloged build(s) in .xorq/builds/:\n"
            )
            ...
            message += "WORK IS NOT DONE UNTIL BUILDS ARE CATALOGED AND COMMITTED!\n\n"
            message += "Use 'xorq catalog ls' to verify cataloged builds"

            payload = {"suppressOutput": False, "decision": "block", "reason": message}
