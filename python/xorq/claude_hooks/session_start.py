#!/usr/bin/env python3
"""SessionStart hook for Claude Code - prints xorq agents onboard to stdout."""
...
def main():
    ...
    try:
        # Get the working directory from environment (set by Claude Code)
        ...
        if (
            not (work_path / ".xorq").exists()
            and not (work_path / ".xorq" / "catalog.yaml").exists()
        ):
