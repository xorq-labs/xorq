...
def test_project_scalar_after_join():
    ...
    con = xo.pandas.connect({"left": left_df, "right": right_df})
    left, right = map(con.table, ("left", "right"))
...
def test_window_with_preceding_expr(index):
    ...
    client = xo.pandas.connect({"df": df})
    t = client.table("df")
    expected = (
        df.set_index("time")
...
def test_rolling_window_udf_nan_and_non_numeric(t, group_by, order_by):
    ...
    assert_series_equal(result_nan, expected, check_names=False)
    assert_series_equal(result_non_numeric, expected, check_names=False)
    assert_series_equal(result_nan_non_numeric, expected, check_names=False)
...
def test_bfill(events):
    con = xo.pandas.connect({"t": events})
    t = con.table("t")
