...
from xorq.backends.pandas.tests.conftest import is_older_than
...
def test_asof_join(time_left, time_right, time_df1, time_df2):
    ...
    with pytest.raises(AssertionError):
        assert_series_equal(result["time"], result["time_right"])
...
def test_keyed_asof_join_with_tolerance(
    ...
    expr = time_keyed_left.asof_join(
        time_keyed_right, "time", predicates="key", tolerance=2 * xo.interval(days=1)
    )
    result = expr.execute()
    expected = pd.merge_asof(
        time_keyed_df1,
    ...
    with pytest.raises(AssertionError):
        assert_series_equal(result["time"], result["time_right"])
    with pytest.raises(AssertionError):
        assert_series_equal(result["key"], result["key_right"])
...
def test_select_on_unambiguous_join(how, func):
    ...
    con = xo.pandas.connect({"t": df_t, "s": df_s})
    t = con.table("t")
    s = con.table("s")
...
def test_outer_join():
    ...
    conn = xo.pandas.connect({"df": df, "df_2": df_2})
...
def test_mutate_after_join():
    ...
    joined = joined.mutate(
        ...
        features=xo.literal("Order_Priority"),
    )
...
def test_chain_join():
    ...
    conn = xo.pandas.connect({"df1": test_df1, "df2": test_df2, "df3": test_df3})
