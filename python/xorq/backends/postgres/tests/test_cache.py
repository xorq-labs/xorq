...
def test_postgres_cache_invalidation(pg, con):
    ...
    expr_cached = (
        ...
        .size()
        .order_by("playerID")
        .cache(cache=SourceCache.from_kwargs(source=con))
    )
    dt = pg_t.op()
    (cache, uncached) = (expr_cached.ls.cache, expr_cached.ls.uncached_one)
...
def test_postgres_snapshot(pg, con):
    ...
    expr_cached = (
        pg_t.group_by("playerID").size().order_by("playerID").cache(cache=cache)
    )
    dt = pg_t.op()
    (cache, uncached) = (expr_cached.ls.cache, expr_cached.ls.uncached_one)
    ...
    with pytest.raises(Exception):  # noqa: B017  # intentional: any exception signals no scan change
        assert_n_scans_changes(dt, n_scans_after)
...
def test_postgres_parquet_snapshot(pg, tmp_path):
    ...
    expr_cached = expr.cache(cache=cache)
    dt = pg_t.op()
    (cache, uncached) = (expr_cached.ls.cache, expr_cached.ls.uncached_one)
