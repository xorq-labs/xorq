...
def test_cross_join_of_pandas_backend_expr(
    left_df: pd.DataFrame, right_df: pd.DataFrame
) -> None:
    """``into_backend`` registers an IbisTableProvider, whose scan can leak.

    The pandas backend's ``to_pyarrow_batches`` carries the blob: it builds the
    result with ``pa.Table.from_pandas`` and ``PyArrowData.convert_table`` skips
    the cast when the types already match, since ``pa.Schema`` equality ignores
    metadata. The provider's declared schema never carries it, so the two would
    disagree.
    """
    pandas_con = xo.pandas.connect({"a": left_df})
    con = xo.connect()
    con.register(pa.Table.from_pandas(right_df), "b")
    remote = pandas_con.table("a").into_backend(con)

    provider = IbisTableProvider(pandas_con.table("a"))
    # check_metadata, since pa.Schema equality ignores it by default
    assert provider.scan().schema.equals(provider.schema(), check_metadata=True)

    expr = remote.cross_join(con.table("b")).group_by("g").aggregate(n=xo._.v.sum())
    assert expr.execute().to_dict("records") == [{"g": "y", "n": 1}]
