...
from xorq.backends.xorq_datafusion.datafusion.compiler import compiler
...
class Backend(SQLBackend, CanCreateCatalog, CanCreateDatabase, CanCreateSchema, NoUrl):
    ...
    def read_postgres(
        ...
        """
        from xorq.backends.postgres import Backend as PostgresBackend  # noqa: PLC0415

        backend = PostgresBackend()
        parsed = urllib.parse.urlparse(uri)
        backend = backend._from_url(parsed, database=database)
        table = backend.table(table_name)
        return self.register_table_provider(table, table_name=table_name)
...
def _polars(source, table_name, _conn, overwrite: bool = False):  # noqa: F811
    tmp_name = gen_name("polars")
    with _create_and_drop_memtable(_conn, table_name, tmp_name, overwrite):
...
def _pyarrow_rb(source, table_name, _conn, overwrite: bool = False):  # noqa: F811
    tmp_name = gen_name("pyarrow")
    with _create_and_drop_memtable(_conn, table_name, tmp_name, overwrite):
