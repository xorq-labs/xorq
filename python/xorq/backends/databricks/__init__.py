...
def _remove_memtable_file(path: str, con) -> None:
...
def _batch_producer(con, sql, chunk_size: int):
    with con.cursor() as cur:
...
def _casting_producer(producer, schema):
...
class Backend(SQLBackend, CanCreateDatabase, UrlFromPath):
    ...
    def do_connect(
        ...
        **config: Any,
        ...
        """Create an xorq client connected to a Databricks cloud instance."""
    ...
    def from_connection(cls, con, /, *, memtable_volume: str | None = None) -> Backend:
        """Create an xorq client from an existing connection to a Databricks cloud instance.
        ...
        memtable_volume
            The volume to use for xorq memtables.
        """
        new_backend = cls()
    ...
    def _post_connect(self, *, memtable_volume: str) -> None:
        sql = f"CREATE VOLUME IF NOT EXISTS `{memtable_volume}` COMMENT 'xorq memtable storage volume'"
        with self.con.cursor() as cur:
            cur.execute(sql)
    ...
    def _register_in_memory_table(self, op: ops.InMemoryTable) -> None:
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
            ...
            # optimize for bandwidth: zstd typically compresses better
    ...
    def read_record_batches(
        ...
        **kwargs: Any,
        ...
        DatabricksADBC(self).adbc_ingest(
            table_name, record_batches, mode=mode, **kwargs
        )
    ...
    def _make_memtable_finalizer(self, name: str) -> Callable[..., None]:
        ...
        return functools.partial(_remove_memtable_file, path, self.con)
    ...
    def to_pyarrow_batches(
        ...
        **_: Any,
        ...
        producer = _casting_producer(
            _batch_producer(self.con, sql, chunk_size), pyarrow_schema
        )
    ...
    def connect_env(cls, **kwargs):
        from xorq.common.utils.databricks_utils import make_connection  # noqa: PLC0415
