...
def test_isnan_isinf(
    ...
    expected = expected_expr_fn(expected_operand_fn(alltypes_df))
...
def test_trig_functions_columns(expr, alltypes, alltypes_df, expected_fn):
    dc_max = alltypes_df.double_col.max()
    expr = alltypes.mutate(dc=(_.double_col / dc_max).nullif(0)).select(tmp=expr)
    result = expr.tmp.to_pandas()
    expected = expected_fn(
        (alltypes_df.double_col / dc_max).replace(0.0, np.nan)
    ).rename("tmp")
    assert_series_equal(result, expected)
...
def test_simple_math_functions_columns(
    con, alltypes, alltypes_df, expr_fn, expected_fn
):
    expr = expr_fn(alltypes).name("tmp")
    expected = expected_fn(alltypes_df)
    result = con.execute(expr)
    assert_series_equal(result, expected)
...
def test_backend_specific_numerics(
    con, alltypes_df, alltypes, expr_fn: Callable, expected_fn: Callable
):
    expr = expr_fn(alltypes)
    result = default_series_rename(con.execute(expr.name("tmp")))
    expected = default_series_rename(expected_fn(alltypes_df))
    assert_series_equal(result, expected)
...
def test_binary_arithmetic_operations(alltypes, alltypes_df, op):
    smallint_col = alltypes.smallint_col + 1  # make it nonzero
    smallint_series = alltypes_df.smallint_col + 1

    expr = op(alltypes.double_col, smallint_col).name("tmp")

    result = expr.execute()
    expected = op(alltypes_df.double_col, smallint_series)
...
def test_mod(alltypes, alltypes_df):
    expr = operator.mod(alltypes.smallint_col, alltypes.smallint_col + 1).name("tmp")

    result = expr.execute()
    expected = operator.mod(alltypes_df.smallint_col, alltypes_df.smallint_col + 1)
    assert_series_equal(result, expected, check_dtype=False)


def test_floating_mod(alltypes, alltypes_df):
    expr = operator.mod(alltypes.double_col, alltypes.smallint_col + 1).name("tmp")

    result = expr.execute()
    expected = operator.mod(alltypes_df.double_col, alltypes_df.smallint_col + 1)
    assert_series_equal(result, expected, check_exact=False)
...
def test_divide_by_zero(alltypes, alltypes_df, column, denominator):
    expr = alltypes[column] / denominator
    result = expr.name("tmp").execute()

    expected = alltypes_df[column].div(denominator)
    expected = expected.astype("float64")
...
def test_bitwise_columns(con, alltypes, alltypes_df, op, left_fn, right_fn):
    expr = op(left_fn(alltypes), right_fn(alltypes)).name("tmp")
    result = con.execute(expr)

    expected = op(left_fn(alltypes_df), right_fn(alltypes_df)).rename("tmp")
    assert_series_equal(result, expected)
...
def test_bitwise_shift(alltypes, alltypes_df, op, left_fn, right_fn):
    expr = op(left_fn(alltypes), right_fn(alltypes)).name("tmp")
    result = expr.execute()

    pandas_left = getattr(left := left_fn(alltypes_df), "values", left)
    pandas_right = getattr(right := right_fn(alltypes_df), "values", right)
    expected = pd.Series(
...
def test_bitwise_not_col(alltypes, alltypes_df):
    expr = (~alltypes.int_col).name("tmp")
    result = expr.execute()
    expected = ~alltypes_df.int_col
...
def test_clip(alltypes, alltypes_df, ibis_func, pandas_func):
    result = ibis_func(alltypes.int_col).execute()
    expected = pandas_func(alltypes_df.int_col).astype(result.dtype)
