...
Covers both publish strategies in ``xorq.sinking.wap``:

  * branch-based (``make_iceberg_branch_wap_expr``): staging branch is
    fast-forwarded into main on pass, retained on fail.
  * table-based (``make_iceberg_wap_expr``): staging table is copied into
...
from xorq.sinking import make_iceberg_branch_wap_expr, make_iceberg_wap_expr
...
def _wap_expr(wap, data, table_name):
    return (
        xo.connect()
...
def test_branch_wap_pass(fresh_con):
    wap = make_iceberg_branch_wap_expr(fresh_con, FINAL)
...
def test_branch_wap_fail(fresh_con):
    wap = make_iceberg_branch_wap_expr(fresh_con, FINAL)
...
def test_branch_wap_append(fresh_con):
    wap = make_iceberg_branch_wap_expr(fresh_con, FINAL)
...
def test_table_wap_pass(fresh_con):
...
def test_table_wap_fail(fresh_con):
    wap = make_iceberg_wap_expr(fresh_con)
    out = _wap_expr(wap, bad, "src_bad").execute()
...
def test_table_wap_append(fresh_con):
    wap = make_iceberg_wap_expr(fresh_con)
    _wap_expr(wap, good, "src1").execute()
    _wap_expr(wap, good, "src2").execute()
