...
# ---- write-primary flag on ThreadedBackendWriteThrough -----------------------


def test_threaded_write_primary_single_call_all_batches() -> None:
    # write-primary still makes ONE streaming ingest carrying every batch, and
    # passes every batch downstream (1:1 pipeline, no fan-out).
    backend = _RecordingBackend()
    writer = ThreadedBackendWriteThrough(
        backend, table_name="t", mode="create", write_primary=True
    )
    batches = [pa.record_batch({"a": [i]}) for i in range(3)]
    out = list(writer.write_through(batches))
    assert [_first(b) for b in out] == [0, 1, 2]
    assert len(backend.calls) == 1
    assert [_first(b) for b in backend.calls[0]["batches"]] == [0, 1, 2]
    assert backend.calls[0]["mode"] == "create"


def test_threaded_write_primary_empty_stream_no_call() -> None:
    backend = _RecordingBackend()
    writer = ThreadedBackendWriteThrough(
        backend, table_name="t", mode="create", write_primary=True
    )
...
def test_threaded_write_primary_completes_on_early_stop() -> None:
    # the write owns the pull loop, so closing the downstream generator early
    # still drives the ingest to completion — drain-always by construction.
    done = threading.Event()

    class _SlowDrain(_RecordingBackend):
        def read_record_batches(self, source, table_name=None, mode=None, **kwargs):
            super().read_record_batches(
                source, table_name=table_name, mode=mode, **kwargs
            )
    ...
    gen = writer.write_through(batches)
    got = [next(gen), next(gen)]
    gen.close()  # blocks until the drain-always write finishes via the join
    assert len(got) == 2
    assert done.is_set()
    assert [_first(b) for b in backend.calls[0]["batches"]] == [0, 1, 2, 3, 4]


def test_threaded_write_primary_error_propagates() -> None:
    class _Exploding(_RecordingBackend):
        def read_record_batches(self, source, table_name=None, mode=None, **kwargs):
            for _ in source:
                raise RuntimeError("simulated ingest failure")
...
def test_threaded_write_primary_identity_neutral(t: Table) -> None:
    # write_primary is transport tuning, not identity-bearing.
    backend = _RecordingBackend()
    down = ThreadedBackendWriteThrough(backend, table_name="t", mode="create")
    up = ThreadedBackendWriteThrough(
        backend, table_name="t", mode="create", write_primary=True
    )
    assert up.__dasher_tokenize__() == down.__dasher_tokenize__()
    assert up == down


def test_threaded_write_primary_build_hash_same(t: Table) -> None:
    target_con = xo.connect()
    down = t.tee(
        ThreadedBackendWriteThrough(target_con, table_name="th_a", mode="create")
    )
    up = t.tee(
        ThreadedBackendWriteThrough(
            target_con, table_name="th_a", mode="create", write_primary=True
        )
    )
    assert get_expr_hash(down) == get_expr_hash(up)


def test_threaded_write_primary_requires_eager_backend(t: Table) -> None:
    # write-primary needs the ingest to DRIVE the pull. datafusion's
    # read_record_batches registers the reader lazily (it pulls only when the
    # target is later queried), so the write never drives the queue and the
    # downstream passthrough comes back empty. Guarded so the broken combo fails
    # loudly instead of silently dropping rows.
    target_con = xo.connect()  # no-mode (lazy) backend
    writer = ThreadedBackendWriteThrough(
        target_con, table_name="wp_tgt", mode="create", write_primary=True
    )
    with pytest.raises(ValueError, match="write_primary requires"):
        t.tee(writer).execute()
