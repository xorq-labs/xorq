...
def test_register_parquet_multiple_paths(con, data_dir):
    fname = "diamonds.parquet"
    table_name = "diamonds"
    diamonds_path = data_dir / "parquet" / fname
    table = con.read_parquet(diamonds_path, table_name=table_name)
    table_multiple_paths = con.read_parquet(
        [
            diamonds_path,
            diamonds_path,
        ],
...
def test_register_csv_files_fails(con, tmp_path):
    ...
    with pytest.raises(
        ValueError, match="does not match the expected extension '.csv'"
    ):
