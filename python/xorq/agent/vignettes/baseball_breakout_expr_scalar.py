...
lag_window = xo.window(group_by="playerID", order_by="yearID", rows=(None, -1))
...
features_engineered = (
    breakout_labels.mutate(
        ...
        prior_breakout=xo._.is_breakout.lag(1).over(
    ...
    .mutate(breakout_label=xo._.is_breakout)
)
...
model_udaf = agg.pandas_df(
    fn=train_model, schema=training_subset.schema(), return_type=dt.binary, name="model"
)
...
train, test = xo.train_test_splits(
    training_subset, test_sizes=0.2, num_buckets=1000, random_seed=42
)
...
predictions = test.mutate(predicted=predict_expr_udf.on_expr(test).name("predicted"))
