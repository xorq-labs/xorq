...
The container's `~/.claude` is isolated from the host. On each entry (`up`, `exec`, `claude`), the host's credentials and permission baseline are re-copied, so the container always reflects your current host permissions. This means any container-side Claude settings changes (e.g. permissions granted during a session) are overwritten on the next entry. Host hooks (global and project-level) are intentionally not copied — they reference host paths and binaries that don't exist inside the container. Only permissions are inherited.
