...
- Markers come from `python/xorq/backends/conftest.py`, applied by path
  ...
  *and* passing `-m core` deselects everything —
  `pytest python/xorq/tests -m core` collects nothing at all, with
  everything deselected. Scope by path or by marker over the whole tree,
  not both; a zero-collection run is the failure mode to watch for.
...
- `ci-test.yml` filters `-k 'not script_execution and not slow'`, but
  ...
  `-k`. The ones named here are not the whole of CI —
  `ls .github/workflows/ci-test*` shows the rest.
...
- Nothing enforces that. The `unlisted-import` rule (category 9) never
  ...
  and `pyproject.toml` configures only `[tool.xorq-style.print]`. Even
  with the rule switched on, a non-literal `__all__` such as
  `python/xorq/ml.py`'s `[*ml.__all__]` stays invisible, because the rule
  reads only statically-evaluable lists. A clean style run is not
  evidence you got `__all__` right.
...
- Don't write a count, total, or percentage into a doc or comment: it's
  a copy of a fact only a command knows, and it goes stale silently with
  nothing checking it. Name the command, or drop the number.
...
- Use `git diff | xorq-check-style --diff` to answer "did I break
  ...
  repo carries thousands of violations, and one `Edit` to
  `python/xorq/cli.py` makes the hook report dozens of them spanning the
